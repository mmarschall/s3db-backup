...
* <tt>ccrypt</tt> (you might need to install it: <tt>apt-get install ccrypt</tt> (Debian, Ubuntu), <tt>yum install ccrypt</tt> (Redhat, CentOS), <tt>brew install ccrypt</tt> (Mac OS X using homebrew))
...
Currently, <em>s3db-backup</em> only supports MySQL databases and works from within a Rails application. Here are the steps to get you started:

* Install the <em>s3db-backup</em> gem

	sudo gem install s3db-backup

* Store your AWS access credentials and your desired S3 buckets in a new file named <tt>config/s3_config.yml</tt>:
...
* Create <tt>db/secret.txt</tt> containing any long and random string as your encryption/decryption key for ccrypt

	$ echo "my secret key" > db/secret.txt
	
* Run your first backup

	$ script/runner S3DbBackup.backup
	
== Restoring your backup using <tt>rake</tt>

To be able to <em>fetch</em> your backups from S3 and <em>load</em> them into your environment (development or production) you can create a set of rake tasks:

* Create <tt>lib/tasks/db.rake</tt> inside your rails application copying this:
...
* Fetch the latest backup from S3 to your local disk:

	$ rake db:seed:fetch
	
* Load that dump into your development environment:
